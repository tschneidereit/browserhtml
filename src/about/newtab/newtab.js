...
const WallpapersAction = action =>( {
  type: 'Wallpapers', source: action
}
);

const TilesAction = action =>( {
  type: 'Tiles', source: action
}
);

const HelpAction = action =>( {
  type: 'Help', source: action
}
);
...
export const init = ():[Model, Effects<Action>] => {
  ...
  return (
      [
        {
          wallpapers, tiles, help
        }, Effects.batch([
                           tilesFx.map(TilesAction), wallpaperFx.map(WallpapersAction), helpFx.map(HelpAction)
                         ])
      ]
  );
}
...
export const view = ({ wallpapers, tiles, help }: Model, address:Address<Action>):DOM => {
  ...
  return (
      html.div({
                 ...
                 thunk('wallpaper', Wallpapers.view, wallpapers, forward(address, WallpapersAction)),
                 thunk('help', Help.view, help, forward(address, HelpAction), activeWallpaper.isDark)
               ])
  )
}
